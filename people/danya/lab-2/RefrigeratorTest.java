...
public class RefrigeratorTest {
    ...
    public static void main(String[] args) {
        ...
        for (Refrigerator ref : refrigerators) {
            String info = ref.getInfo();
            if (ref instanceof RefrigeratorWithFreezer rwf) {
                // Компактная строка для холодильника с морозилкой
                System.out.printf("%s: %s, %dx%dx%d см, %s, %d кам., мороз. %.1f л%n",
                        ref.getManufacturer(),
                        ref.getColor(),
                        (int)ref.getHeight(), (int)ref.getWidth(), (int)ref.getDepth(),
                        rwf.getChamberType().getName(),
                        rwf.getNumberOfChambers(),
                        rwf.getFreezerVolume());
            } else {
                // Компактная строка для обычного холодильника
                System.out.printf("%s Холодильник: %s, %dx%dx%d см (%.1f л)%n",
                        ref.getManufacturer(),
                        ref.getColor(),
                        (int)ref.getHeight(), (int)ref.getWidth(), (int)ref.getDepth(),
                        ref.calculateVolume() / 1000.0);
            }
        }
       ...
       System.out.println("\n=== ХОЛОДИЛЬНИКИ С ОБЪЕМОМ МОРОЗИЛЬНОЙ КАМЕРЫ БОЛЬШЕ " + 
                  minFreezerVolume + " Л ===\n");
        for (Refrigerator ref : refrigerators) {
            if (ref instanceof RefrigeratorWithFreezer rwf && rwf.getFreezerVolume() > minFreezerVolume) {
                System.out.printf("%s: %s, %dx%dx%d см, %s, %d кам., мороз. %.1f л%n",
                        ref.getManufacturer(),
                        ref.getColor(),
                        (int)ref.getHeight(), (int)ref.getWidth(), (int)ref.getDepth(),
